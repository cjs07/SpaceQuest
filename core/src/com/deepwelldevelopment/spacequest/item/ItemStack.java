...
public class ItemStack {
            ...
            if (item.getSprite() == null) {
                if (item instanceof ItemBlock) {
                    if (((ItemBlock) item).getBlock() == BlockProvider.air) {
                        System.out.println("null sprite, but the block was not air, block had id " +
                                ((ItemBlock) item).getBlock().getId());
                    }
                }
            }
            if (item.getSprite() != null) {
                ...
                //TODO: apply a missing texture for items with no texture
            }
        }
    }
    ...
    public ItemStack(Block block, int amount) {
        this(Item.getItemFromBlock(block), amount);
    }
    ...
    public void render(Batch batch, float x, float y, float slotX, float slotY, float slotSize) {
        ...
        if (slotSize > 0) {
            ...
            sprite.draw(batch);
            SpaceQuest.getSpaceQuest().getFont().draw(batch, stackSize + "", slotX + slotSize,
                    slotY + slotSize
            );
        } else {
