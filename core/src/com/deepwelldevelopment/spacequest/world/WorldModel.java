...
public class WorldModel implements RenderableProvider {

    public static final int CHUNK_SIZE_X = 16;
    public static final int CHUNK_SIZE_Y = 16;
    public static final int CHUNK_SIZE_Z = 16;

    public final ChunkModel[] chunks;
    ...
    public WorldModel(TextureRegion[] tiles, int chunksX, int chunksY, int chunksZ) {
        this.tiles = tiles;
        this.chunks = new ChunkModel[chunksX * chunksY * chunksZ];
        ...
        for (int y = 0; y < chunksY; y++) {
            for (int z = 0; z < chunksZ; z++) {
                for (int x = 0; x < chunksX; x++) {
                    ChunkModel chunk = new ChunkModel(CHUNK_SIZE_X, CHUNK_SIZE_Y, CHUNK_SIZE_Z);
        ...
        this.vertices = new float[ChunkModel.VERTEX_SIZE * 6 * CHUNK_SIZE_X * CHUNK_SIZE_Y * CHUNK_SIZE_Z];
    ...
    public void getRenderables(Array<Renderable> renderables, Pool<Renderable> pool) {
        ...
        for (int i = 0; i < chunks.length; i++) {
            ChunkModel chunk = chunks[i];
            Mesh mesh = meshes[i];
            if (dirty[i]) {
                int numVerts = chunk.calculateVertices(vertices);
                numVertices[i] = numVerts / 4 * 6;
                mesh.setVertices(vertices, 0, numVerts * ChunkModel.VERTEX_SIZE);
                dirty[i] = false;
            }
