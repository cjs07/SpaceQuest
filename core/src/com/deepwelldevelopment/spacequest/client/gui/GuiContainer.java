...
/**
 * Represents a GUI that displays the contents of a container or some other GUI that has storage
...
public class GuiContainer extends Gui {
    ...
    /** The stack currently attached to the mouse */
    ...
    public GuiContainer(Container container, String texture) {
        ...
        if (hoveredSlotSprite == null) {
            hoveredSlotSprite = SpaceQuest.getSpaceQuest().getTextureAtlas().createSprite(
                    "gui_slot_hovered");
            hoveredSlotSprite.setSize(hoveredSlotSprite.getWidth() * GUI_SCALE,
    ...
    public void render(Batch batch, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < container.getSlots().size(); i++) {
            ...
            if (slot.getStack() != ItemStack.EMPTY) {
                slot.getStack().render(batch, x + slot.getX() + (8 * GUI_SCALE) -
                                (slot.getStack().getSprite().getWidth() / 2),
                        y + slot.getY() + (8 * GUI_SCALE) -
                                (slot.getStack().getSprite().getHeight() / 2),
